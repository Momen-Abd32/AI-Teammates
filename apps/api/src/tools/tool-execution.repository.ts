...
export class ToolExecutionRepository{
 ...
 async complete(id:string,status:string,result:unknown){
  const r=await this.db.query(`UPDATE tool_executions SET status=$1,result=$2::jsonb,completed_at=now() WHERE id=$3
   RETURNING id,status,result,completed_at AS "completedAt"`,[status,JSON.stringify(result),id]);
  return r.rows[0];
 }
 async get(id:string){const r=await this.db.query(`SELECT id,company_id AS "companyId",agent_id AS "agentId",action,resource,arguments,status,result,created_at AS "createdAt",completed_at AS "completedAt" FROM tool_executions WHERE id=$1`,[id]);return r.rows[0];}
}
