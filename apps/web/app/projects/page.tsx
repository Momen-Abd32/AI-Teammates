...
export default function Projects(){
 ...
 const load=()=>apiFetch("/projects/company/company-demo").then(r=>r.json()).then(setProjects);
 useEffect(()=>{load().catch(()=>{});},[]);
 async function create(){if(!name.trim())return;await apiFetch("/projects",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({companyId:"company-demo",name})});setName("");load();}
